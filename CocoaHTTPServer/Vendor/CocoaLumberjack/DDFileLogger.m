...
- (void)logMessage:(DDLogMessage *)logMessage
{
	...
	if (logMsg)
	{
		...
		@try {
			if (droppedMessageCount) {
				// once the disk has some space again, warn about dropped messages
				// prepend \n in case last successful log message was truncated
				NSString *droppedMsg = [NSString stringWithFormat:@"\n--- dropped %lu message%@ due to insufficient disk space ---\n", droppedMessageCount, droppedMessageCount == 1 ? @"" : @"s"];
				NSData *droppedData = [droppedMsg dataUsingEncoding:NSUTF8StringEncoding];
				[[self currentLogFileHandle] writeData:droppedData];
				// reset count only on success
				droppedMessageCount = 0;
			}
		...
		@catch (NSException *e) {
			// unfortunately NSFileHandleOperationExceptions contain only a reason string and not an errno so match by string
			if ([[e name] isEqualToString:NSFileHandleOperationException] &&
				[[e reason] rangeOfString:[NSString stringWithUTF8String:strerror(ENOSPC)]].location != NSNotFound) {
				++droppedMessageCount;
			} else {
