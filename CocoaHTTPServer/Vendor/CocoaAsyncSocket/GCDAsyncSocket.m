...
- (id)initWithData:(NSData *)d timeout:(NSTimeInterval)t tag:(long)i countBytesWritten:(BOOL)c;
@end
...
/**
 * The GCDAsyncSendfilePacket encompasses the instructions for a write using sendfile(2)
**/
@interface GCDAsyncSendfilePacket : GCDAsyncWritePacket
{
  @public
	NSFileHandle *file;
	off_t offset;
	off_t length; /* Weird, but sendfile(2) uses off_t for length */
}

- (id)initWithSendfile:(NSFileHandle *)f offset:(off_t)o length:(off_t)l timeout:(NSTimeInterval)t tag:(long)i countBytesWritten:(BOOL)c;
@end
...
- (id)initWithSendfile:(NSFileHandle *)f offset:(off_t)o length:(off_t)l timeout:(NSTimeInterval)t tag:(long)i countBytesWritten:(BOOL)c;
{
	if((self = [super initWithData:nil timeout:t tag:i countBytesWritten:c]))
	{
		file = f;
		offset = o;
		length = l;
...
- (NSError *)connectTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"GCDAsyncSocketConnectTimeoutError",
	                                                     @"GCDAsyncSocket", [NSBundle bundleWithIdentifier:@"com.apple.HTTPServer"],
	                                                     @"Attempt to connect to host timed out", nil);
	if (errMsg == nil)
		errMsg = @"Attempt to connect to host timed out";
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
...
- (NSError *)readMaxedOutError
{
	...
	if (errMsg == nil)
		errMsg = @"Read operation reached set maximum length";
...
- (NSError *)readTimeoutError
{
	...
	if (errMsg == nil)
		errMsg = @"Read operation timed out";
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
...
- (NSError *)writeTimeoutError
{
	...
	if (errMsg == nil)
		errMsg = @"Write operation timed out";
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
...
- (NSError *)connectionClosedError
{
	...
	if (errMsg == nil)
		errMsg = @"Socket closed by remote peer";
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
...
- (NSArray *)localAddresses
{
	...
	dispatch_block_t block = ^{
		if (socket4FD != SOCKET_NULL)
		{
			...
			if (getsockname(socket4FD, (struct sockaddr *)&sockaddr4, &sockaddr4len) == 0)
			{
				[addresses addObject:[[NSData alloc] initWithBytes:&sockaddr4 length:sockaddr4len]];
			}
		}
		...
		if (socket6FD != SOCKET_NULL)
		{
			...
			if (getsockname(socket6FD, (struct sockaddr *)&sockaddr6, &sockaddr6len) == 0)
			{
				[addresses addObject:[[NSData alloc] initWithBytes:&sockaddr6 length:sockaddr6len]];
			}
		}
	};
...
- (void)asyncQueuePacket:(GCDAsyncWritePacket *)packet
{
	dispatch_async(socketQueue, ^{ @autoreleasepool {
...
{
	[self writeData:data withTimeout:timeout tag:tag countBytesWritten:NO];
}
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag countBytesWritten:(BOOL)countBytesWritten
{
	...
	GCDAsyncWritePacket *packet = [[GCDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag countBytesWritten:countBytesWritten];
	
	[self asyncQueuePacket:packet];
}
...
{
	[self writeSendfile:file atOffset:offset andLength:length withTimeout:timeout tag:tag countBytesWritten:NO];
}
...
- (void)writeSendfile:(NSFileHandle *)file atOffset:(off_t)offset andLength:(off_t)length withTimeout:(NSTimeInterval)timeout tag:(long)tag countBytesWritten:(BOOL)countBytesWritten
{
	...
	GCDAsyncSendfilePacket *packet = [[GCDAsyncSendfilePacket alloc] initWithSendfile:file offset:offset length:length timeout:timeout tag:tag countBytesWritten:countBytesWritten];
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			LogVerbose(@"Dequeued %@", currentWrite);
...
- (void)doWriteData
{
	...
	if (flags & kSocketSecure)
	{
		...
		// XXX TODO: if the current write is a sendfile packet, call read
		// and put data in buffer since SSL can't use sendfile
		NSAssert(![currentWrite isKindOfClass:[GCDAsyncSendfilePacket class]], @"Secure sockets cannot use sendfile");
	...
	else if ([currentWrite isKindOfClass:[GCDAsyncSendfilePacket class]])
	{
		GCDAsyncSendfilePacket *currentSendfile = (GCDAsyncSendfilePacket *)currentWrite;
		int socketFD = (socket4FD == SOCKET_NULL) ? socket6FD : socket4FD;
		
		off_t len = currentSendfile->length - currentSendfile->bytesDone;
		assert(len > 0); // The len 0 is special for sendfile, so make sure we don't accidentally use it.

		int result = sendfile([currentSendfile->file fileDescriptor], socketFD, currentSendfile->offset + currentSendfile->bytesDone, &len, NULL, 0);
		LogVerbose(@"sendfile %d to %d at off %lld len %lld, result = %d, errno = %d, len = %lld",
			[currentSendfile->file fileDescriptor], socketFD,
			currentSendfile->offset + currentSendfile->bytesDone,
			currentSendfile->length - currentSendfile->bytesDone,
			result, errno, len);
		...
		if (result < 0)
		{
			...
			{
				error = [self errnoErrorWithReason:@"Error in sendfile() function"];
			}
		}
		else
		{
	...
	if (bytesWritten > 0)
	{
		...
		BOOL doCountBytesWritten = NO;
		if ([currentWrite isKindOfClass:[GCDAsyncSendfilePacket class]])
			doCountBytesWritten = ((GCDAsyncSendfilePacket *) currentWrite)->countBytesWritten;
		else
			doCountBytesWritten = currentWrite->countBytesWritten;
		if (doCountBytesWritten)
			countedBytesWritten += bytesWritten;

		// Is packet done?
		if ([currentWrite isKindOfClass:[GCDAsyncSendfilePacket class]]) {
			done = (currentWrite->bytesDone == ((GCDAsyncSendfilePacket *)currentWrite)->length);
		} else {
