...
/* Return the number of bytes actually written which were requested to be counted via
   writeData:withTimeout:tag:countBytesWritten: or writeSendfile:atOffset:andLength:withTimeout:tag:countBytesWritten.
   countedBytesWritten can't be reset but it can be read at different times and subtracted. */
- (UInt64)countedBytesWritten;
...
/**
 * Writes data to the socket using sendfile(2) and calls the delegate when finished.
 *
 * Same as writeData, except reads the data from file at offset and length.
**/
 ...
 * 
 * If SecureTransport is available on iOS or Mac OS X >= 10.9:
 * 
