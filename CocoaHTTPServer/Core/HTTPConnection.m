...
- (HTTPData *)readDataOfLength:(NSUInteger)length orLimit:(NSUInteger)limit;
- (void)writeData:(HTTPData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag;
@end
...
- (void)startConnection
{
	...
	if ([self isSecureServer])
	{
		...
		if ([certificates count] > 0)
		{
			...
			// Allow SSLHandhake to complete so that manual evaluation gets a chance 
            if ( [[asyncSocket delegate] respondsToSelector:@selector(shouldManuallyEvaluatePeerTrustForSocket:)]
                && [[asyncSocket delegate] shouldManuallyEvaluatePeerTrustForSocket:asyncSocket])
            {
                HTTPLogInfo(@"Setting kCFStreamSSLValidatesCertificateChain to NO");
...
- (BOOL)parseRangeRequest:(NSString *)rangeHeader withContentLength:(UInt64)contentLength
{
	...
	NSMutableArray *xranges = [[NSMutableArray alloc] initWithCapacity:[rangeComponents count]];
	...
	for (i = 0; i < [rangeComponents count]; i++)
	{
		...
		if (dashRange.location == NSNotFound)
		{
			...
			[xranges addObject:[NSValue valueWithDDRange:DDMakeRange(byteIndex, 1)]];
		}
		else
		{
			...
			if (!hasR1)
			{
				...
				[xranges addObject:[NSValue valueWithDDRange:DDMakeRange(startIndex, r2)]];
			}
			else if (!hasR2)
			{
				// We're dealing with a "[#]-" range
				...
				[xranges addObject:[NSValue valueWithDDRange:DDMakeRange(r1, contentLength - r1)]];
			}
			else
			{
				...
				[xranges addObject:[NSValue valueWithDDRange:DDMakeRange(r1, r2 - r1 + 1)]];
			}
		}
	}
	
	if([xranges count] == 0) return NO;
	
	// Now make sure none of the ranges overlap
	
	for (i = 0; i < [xranges count] - 1; i++)
	{
		DDRange range1 = [[xranges objectAtIndex:i] ddrangeValue];
		
		NSUInteger j;
		for (j = i+1; j < [xranges count]; j++)
		{
			DDRange range2 = [[xranges objectAtIndex:j] ddrangeValue];
	...
	[xranges sortUsingSelector:@selector(ddrangeCompare:)];
	ranges = xranges;	// set ranges only on success, or else responseHasAvailableData will do the wrong thing
...
- (void)sendResponseHeadersAndBody
{
	...
	else
	{
		...
		{
			// Regular request
			HTTPData *data = [self readDataOfLength:NSUIntegerMax orLimit:READ_CHUNKSIZE];
			...
			if ([data length] > 0)
			{
				...
				if (isChunked)
				{
					...
					[self writeData:data withTimeout:TIMEOUT_WRITE_BODY tag:HTTP_CHUNKED_RESPONSE_BODY];
				...
				else
				{
					...
					[self writeData:data withTimeout:TIMEOUT_WRITE_BODY tag:tag];
				}
			}
		}
		else
		{
			...
			if ([ranges count] == 1)
			{
				...
				HTTPData *data = [self readDataOfLength:range.length orLimit:READ_CHUNKSIZE];
			...
			else
			{
				...
				if ([data length] > 0)
				{
					...
					[self writeData:data withTimeout:TIMEOUT_WRITE_BODY tag:HTTP_PARTIAL_RANGES_RESPONSE_BODY];
				}
			}
		}
	}
...
- (void)continueSendingStandardResponseBody
{
	...
	if(writeQueueSize < READ_CHUNKSIZE) {
		available = READ_CHUNKSIZE - writeQueueSize;
	}
	
	HTTPData *data = [self readDataOfLength:NSUIntegerMax orLimit:available];
...
- (void)continueSendingSingleRangeResponseBody
{
	...
	if (bytesLeft > 0)
	{
		HTTPData *data = [self readDataOfLength:bytesLeft orLimit:available];
...
- (void)continueSendingMultiRangeResponseBody
{
	...
	else
	{
		if (++rangeIndex < [ranges count])
		{
			...
			HTTPData *data = [self readDataOfLength:range.length orLimit:available];
...
{
	HTTPLogWarn(@"HTTP Server: Error 501 - Not Implemented (%@)", [self requestURI]);
...
{
	HTTPLogWarn(@"HTTP Server: Error 503 - Service Unavailable (%@)", [self requestURI]);
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err
{
	...
	// save this value before we release the socket
	posthumousCountedBytesWritten = [asyncSocket countedBytesWritten];
...
- (void)responseHasAvailableData:(NSObject<HTTPResponse> *)sender
{
	...
	dispatch_async(connectionQueue, ^{ @autoreleasepool {
		...
		{
			HTTPLogWarn(@"%@[%p]: %@ - Sender %p is not current httpResponse %p", THIS_FILE, self, THIS_METHOD, sender, httpResponse);
			return;
		}
...
{
	// return a meaningful value even if the asyncSocket is gone
	return asyncSocket ? [asyncSocket countedBytesWritten] : posthumousCountedBytesWritten;
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark HTTPData support
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

- (HTTPData *)readDataOfLength:(NSUInteger)length orLimit:(NSUInteger)limit
{
	if ([httpResponse respondsToSelector:@selector(readHTTPDataOfLength:orLimit:)]) {
		return [httpResponse readHTTPDataOfLength:length orLimit:limit];
	}
	if (length > limit) {
		length = limit;
	}
	...
	return [[HTTPData alloc] initWithData:[httpResponse readDataOfLength:length]];
}
...
- (void)writeData:(HTTPData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	if (nsdata) {
		[asyncSocket writeData:nsdata withTimeout:TIMEOUT_WRITE_BODY tag:tag countBytesWritten:YES];
	} else {
		[asyncSocket writeSendfile:data.file atOffset:data.offset andLength:data.length withTimeout:TIMEOUT_WRITE_BODY tag:tag countBytesWritten:YES];
	}
}
...
{
	NSAssert(aOffset >= 0, @"Sendfile cannot use negative offset");
	NSAssert(aLength >= 0, @"Sendfile cannot use negative length");
